...
names(inds)
```
By changing arguments such that `raw = FALSE` and `std = TRUE`, the function call will return the standarized versions (denoted with `_s` in the indicator name) of the same structure and functioning indicators:
...
Finally, the function `extractAll()` can calculate all indicators. The user can choose whether the function returns the indicator dataframe to the global environment, or exports the dataframe to a csv file. The user can also choose whether the function returns the raw indicator values, the standaradized (z-score) values, or both. The following example calculates 53 standardized indicators and returns them to the global environment in variable all_inds.
